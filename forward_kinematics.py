...
def forward_kinematics(joints):
    ...
    M1 = mr.MatrixExp6(mr.VecTose3(S1 * joint1))
    M2 = mr.MatrixExp6(mr.VecTose3(S2 * joint2))
    M3 = mr.MatrixExp6(mr.VecTose3(S3 * joint3))

    # Calculate end effector transformation matrix
    T = np.dot(np.dot(np.dot(M1, M2), M3), np.array([[1, 0, 0, (link3x + link4x)],
                                                     [0, 1, 0, 0],
